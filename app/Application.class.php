...
**/
	...
	public function browserIsFireFox(){
		if (isset($_SERVER['HTTP_USER_AGENT']) && 
	...
	public function browserIschrome(){
	...
	public function d($d){// array dump
    echo '<pre>';
    print_r($d);
    echo '</pre>';
	}
	
	public function getGeoCoord($address){
	...
	//	echo "<pre>";print_r($retValue);echo "</pre>";
	
		$jsonArray=json_decode($retValue,true);
		
	//	echo "<pre>";print_r($jsonArray);echo "</pre>";
		
		$geo['lat'] =  $jsonArray["results"][0]["geometry"]["location"]["lat"];
		$geo['lng'] =  $jsonArray["results"][0]["geometry"]["location"]["lng"];
	...
	public function truncateThis($string, $limit, $break=" ", $pad="...") { // return with no change if string is shorter than $limit  
	...
	public function GetMyLocation(){// Get My Location by IP Address
	...
	public function CheckMyLocation(){// Check My Current Location
		...
		{//
			...
			parent::SetQuery("
					SELECT * FROM table_offerlocations,
												table_offers,
												aggregate_deal_location
					WHERE					table_offerlocations.offer_id = table_offers.offer_id
					AND 					table_offerlocations.location_id  = aggregate_deal_location.location_id
					");
				...
				if( $exists )
				{
					...
					{// loop through each location
						...
						{// make sure city name is not blank
							parent::SetQuery("SELECT * FROM `aggregate_deal_location` WHERE location LIKE '%{$location["city_name"]}%'");
							//parent::PrintQuery();
		...
		{// set a default location
			parent::SetQuery("SELECT * FROM aggregate_deal_location ORDER BY location ASC LIMIT 1");
...
**/
		...
		{// deals found
			...
			{// loop through each deal
				...
				<tr>
					...
					<td><?=$deal["title"];?></td>
					<td><?php
						parent::SetQuery("SELECT * FROM `aggregate_deal_location` WHERE 
	...
	<?php } ?>
		...
		<?php
			parent::SetQuery("SELECT * FROM `aggregate_deal_location`");
			$locations = parent::DoQuery();
			foreach( $locations as $location )
			{// loop through each location
			?>
		...
		<?php
			parent::SetQuery("SELECT * FROM `aggregate_deal_source`");
			$sources = parent::DoQuery();
			foreach( $sources as $source )
			{// loop through each source
			?>
				<option value="<?=$source["aggregate_deal_source_id"];?>" <?php if( $deal_info["deal_source"] == $source["source_id"] ) { ?>selected="selected"<?php } ?>><?=$source["source_name"];?></option>
			<?php
			}
		?>
		</select>
	...
	<?php } ?>
		...
		<select name="deal[source]" style="width: 250px;">
		<?php
			parent::SetQuery("SELECT * FROM `aggregator_deal_source`");
			$sources = parent::DoQuery();
			foreach( $sources as $source )
			{// loop through each source
			?>
	...
	{// do add the deal source to the database
		if( $this->LocalCheckAuth() )
		{// admin is logged in
			parent::SetQuery("INSERT INTO `aggregate_deal_source` VALUES ('','{$this->localData["source"]["name"]}','{$this->localData["source"]["url"]}')");
			parent::SimpleQuery();
	...
	public function GetAggregatedDeals( $myLocation ){// retrieve aggregated deals
		
		$this->rowsPerPage = 10;

		# Create Paging
		parent::SetQuery( "	SELECT * 
												FROM 	aggregate_deal, aggregate_deal_location 
												WHERE aggregate_deal_region_id ='6' 
												AND		aggregate_deal_location.aggregate_deal_id = aggregate_deal.aggregate_deal_id
												AND 	aggregate_deal.price > 0 
											" );
												
		parent::SetQuery( "	SELECT * 
												FROM 	aggregate_deal, aggregate_deal_source
												WHERE aggregate_deal_region_id ='6' 
												AND 	aggregate_deal.price > 0
												AND		aggregate_deal.aggregate_deal_source_id = aggregate_deal_source.aggregate_deal_source_id 
												limit 3" );													
		parent::SetQuery( "	SELECT * 
												FROM 	aggregate_deal, aggregate_deal_location, aggregate_deal_source
												WHERE aggregate_deal_region_id ='6' 
												AND 	aggregate_deal.price > 0
												AND		aggregate_deal.aggregate_deal_id = aggregate_deal_location.aggregate_deal_id 
												AND		aggregate_deal.aggregate_deal_source_id = aggregate_deal_source.aggregate_deal_source_id 
												" );		
										
																			
											
		$dataArray[totalDeals] = $totalDeals = parent::CountDBResults();
		$dataArray[paging] = $paging = $this->Pagination( array( "totalRows" => $totalDeals ) );
		$dataArray[deals] = parent::DoQuery();

		return $dataArray;
		
//		parent::SetQuery( "SELECT * FROM `aggregate_deal` WHERE aggregate_deal_region_id ='{$this->localData["location_id"]}' AND `price`>0" );
//		parent::SetQuery("SELECT * FROM `aggregate_deal` WHERE aggregate_deal_region_id='{$this->localData["location_id"]}' AND `price`>0 LIMIT {$this->localData["s"]},{$this->rowsPerPage}");
//		parent::SetQuery("SELECT * FROM `aggregate_deal` WHERE aggregate_deal_region_id='6' AND `price`>0 LIMIT {$this->localData["s"]},{$this->rowsPerPage}");
	}
	

	public function GetAggregatedDeal( $aggregate_deal_id  ){

		parent::SetQuery( "	SELECT * 
												FROM 	aggregate_deal, aggregate_deal_location 
												WHERE aggregate_deal.aggregate_deal_id = $aggregate_deal_id 
												AND		aggregate_deal_location.aggregate_deal_id = aggregate_deal.aggregate_deal_id
												AND 	aggregate_deal.price > 0 
											" );
	...
	public function insertAggregateDealReferred( $aggregate_deal_source_id  ){

		parent::SetQuery("INSERT INTO aggregate_deal_referred (
																														date,
	...
	<?php } ?>
		...
		<?php
			...
			<?php
				...
				{# Loop Through Each Offer
					parent::SetQuery("SELECT * FROM aggregate_deal_location WHERE location_id='{$subscriber["location_id"]}' LIMIT 1");
					$locationData = parent::DoQuery();
...
**/
	...
	{# View Locations
		...
		parent::SetQuery( "SELECT * FROM aggregate_deal_location ORDER BY location ASC LIMIT {$this->localData['s']},{$this->rowsPerPage}" );
	...
	{# Process Add Location Form
		...
		{
			parent::SetQuery("INSERT INTO aggregate_deal_location VALUES ('','{$this->localData['location']}')");
	...
	{# Remove Location
		if( $this->LocalCheckAuth() )
		{// check if admin logged in
		parent::SetQuery("SELECT * FROM aggregate_deal_location WHERE location_id='{$this->localData['location_id']}' LIMIT 1");
		$locationData = parent::DoQuery();
		// Remove Location from Location Table
		parent::SetQuery("DELETE FROM aggregate_deal_location WHERE location_id='{$this->localData['location_id']}' LIMIT 1");
		parent::SimpleQuery();
...
**/
	...
	{// Recent Deals Page
		...
		parent::SetQuery("SELECT * FROM `aggregate_deal_location` WHERE aggregate_deal_region_id='{$this->localData["location_id"]}'");
	...
	{// get offer locations
		parent::SetQuery("SELECT * FROM `table_offerlocations`,`aggregate_deal_location` WHERE 
		`table_offerlocations`.`location_id`=`aggregate_deal_location`.`location_id`
		AND
